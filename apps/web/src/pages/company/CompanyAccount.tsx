...
export default function CompanyAccount() {
  ...
  async function handleLogout() {
    ...
    try {
      ...
      // backend logout stateless: vẫn gọi để đúng flow, nhưng client sẽ xóa token
  ...
  const displayName = me?.name || "Đại diện công ty";
  const initials = (displayName || "R").trim().slice(0, 1).toUpperCase();

  const companyStatusClass =
    me?.companyStatus === "ACTIVE"
      ? "isActive"
      : me?.companyStatus === "PENDING"
      ? "isPending"
      : me?.companyStatus === "REJECTED"
      ? "isRejected"
      : "isUnknown";
              ...
              <>
                ...
                <div className="caCard">
                  <div className="caCardHeader">
                    ...
                    <div className="caPills">
                      <span className="caPill">{roleLabel(me?.role)}</span>
                    </div>
                  </div>
                ...
                <div className="caCard">
                  <div className="caCardTop">
                    <div>
                      <div className="caCardTitle">Thông tin công ty</div>
                      <div className="caMuted">Trạng thái duyệt &amp; tên hiển thị</div>
                    </div>
                  </div>
                  ...
                  <div className="caRows">
                    <div className="caRow">
                      <div className="caLabel">Tên công ty</div>
                      <div className="caValue">{me?.companyName || "—"}</div>
                    </div>

                    <div className="caRow">
                      <div className="caLabel">Trạng thái</div>
                      <div className={`caStatus ${companyStatusClass}`}>
                        {statusLabel(me?.companyStatus)}
                      </div>
                    </div>
                  </div>

                  <div className="caActions">
                    <button
                    ...
                    <button className="btnPrimary" onClick={() => navigate("/company/feedback")}>
                      Đọc phản hồi
                    </button>
                  </div>
                </div>
